...
const controlledDemoTemplates = [
  {
    ...
    intent: { label: 'Teacher lesson-preparation workflow', summary: 'Turn a lesson topic, grade level, and learning objective into a repeatable research-and-review brief. The teacher reviews all material before classroom use.', method: 'Teacher-declared objective', safeguards: ['Teacher selects and verifies trusted sources.', 'Teacher reviews activities and learner supports.', 'Teacher approves the brief before classroom use.'] },
    code: "import { test, expect } from '@playwright/test';\n\ntest('build a teacher lesson-plan research brief', async ({ page }) => {\n  await page.goto(process.env.ANUKRITI_CONTROLLED_DEMO_ORIGIN + '/demo-websites/anukriti-teacher-lesson-brief.html');\n  await page.getByLabel('Grade level').fill('Grade 7');\n  await page.getByLabel('Lesson topic').fill('Water cycle');\n  await page.getByLabel('Learning objective').fill('Explain evaporation, condensation, and precipitation');\n  await page.getByLabel('Lesson length').selectOption('45');\n  await page.getByLabel('Learner support').selectOption('visual');\n  await page.getByRole('button', { name: 'Build lesson brief' }).click();\n  await expect(page.getByRole('heading', { name: 'Teacher lesson brief ready' })).toBeVisible();\n  await expect(page.getByText('Teacher review required before classroom use.')).toBeVisible();\n});\n"
  },
  {
    key: 'anukriti-study-brief', name: 'Demo: Study brief builder', startPath: '/demo-websites/anukriti-study-brief.html',
    intent: { label: 'Student study workflow', summary: 'Turn a topic and learning goal into a repeatable find → explain → check routine. The student owns the explanation and brings gaps to a teacher or peer.', method: 'Student-declared learning goal', safeguards: ['Use trusted sources chosen by the learner or teacher.', 'Keep explanations in the learner’s own words.', 'Review gaps with a teacher or peer before treating the work as complete.'] },
    code: "import { test, expect } from '@playwright/test';\n\ntest('build a study brief', async ({ page }) => {\n  await page.goto(process.env.ANUKRITI_CONTROLLED_DEMO_ORIGIN + '/demo-websites/anukriti-study-brief.html');\n  await page.getByLabel('Study topic').fill('Photosynthesis');\n  await page.getByLabel('Learning goal').fill('Explain the process in my own words');\n  await page.getByLabel('What I already know').fill('Plants use sunlight.');\n  await page.getByLabel('Source preference').selectOption('library');\n  await page.getByRole('button', { name: 'Build study brief' }).click();\n  await expect(page.getByRole('heading', { name: 'Study brief ready' })).toBeVisible();\n  await expect(page.getByText('Explain it in your own words before checking the answer.')).toBeVisible();\n});\n"
  },
  {
...
export function createControlledDemoWorkflows(db, ownerId, controlledDemoOrigin) {
  ...
  for (const template of controlledDemoTemplates) {
    ...
    if (template.intent) workflow.intent = { ...intentFor(workflow), ...template.intent };
