...
`);
  ...
  await page.getByRole('button', { name: 'Close personal assistant' }).click();
  await page.getByRole('button', { name: 'Numbers bridge' }).click();
  await page.getByRole('heading', { name: 'Research in a browser. Return proof to Numbers.' }).waitFor({ timeout: 8000 });
  assert.match(await page.locator('.numbersResearch').innerText(), /Anukriti Research Input[\s\S]*Anukriti Research Results/);
  await page.getByRole('button', { name: 'Capture active Numbers research template →' }).waitFor({ timeout: 8000 });
  ...
  await page.getByRole('button', { name: 'Resume tailor' }).click();
  await page.getByRole('heading', { name: 'Align your resume without inventing it.' }).waitFor({ timeout: 8000 });
  await page.locator('input[type="file"]').setInputFiles({ name: 'ada-resume.txt', mimeType: 'text/plain', buffer: Buffer.from('Ada Lovelace\nProduct manager with product management, data analysis, stakeholder management, and cross functional leadership experience.') });
  await page.getByLabel('Job description').fill('Senior Product Manager\n- Product management and data analysis\n- Stakeholder management and cross functional leadership\n- SQL and market research');
  await page.getByRole('button', { name: 'Analyze alignment →' }).click();
  await page.getByText(/Alignment review ready/).waitFor({ timeout: 8000 });
  assert.match(await page.locator('.resumeMatrix').innerText(), /Evidenced[\s\S]*Not evidenced/);
  await page.getByRole('button', { name: 'Generate separate Word review copy →' }).click();
  await page.getByText('Separate Word review copy ready.').waitFor({ timeout: 8000 });
  const resumeProofDownload = page.waitForEvent('download');
  await page.getByRole('button', { name: 'Download proof' }).click();
  assert.match((await resumeProofDownload).suggestedFilename(), /resume-alignment-proof\.json/);
  const resumeDownload = page.waitForEvent('download');
  await page.getByRole('button', { name: 'Download .docx →' }).click();
  assert.match((await resumeDownload).suggestedFilename(), /resume-alignment-.*\.docx/);
